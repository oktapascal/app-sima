...
import {IconWindowClose, IconMoon, IconLogout} from "@/components";
import instance from "@/api/instance";
import {useAlertStore} from "@/stores/alert";
import type {AxiosResponse} from "axios";
import type {IAlert, IUser, IUserResponse} from "@/types";
...
async function onFetchDataUser() {
  try {
    const response: AxiosResponse<IUserResponse> = await instance.get("/profile/user");
...
<template>
  <div class="absolute top-0 h-screen w-full bg-gray-100 dark:bg-black z-50">
    <div class="relative flex flex-col">
      <div class="flex-none">
        <button class="focus:outline-none font-medium rounded-full p-2 inline-flex items-center"
                @click="onCloseProfileBox">
      ...
      <div class="flex-1">
        <div class="flex flex-col p-2.5">
          ...
          <div class="flex-1 mt-2">
            <button type="button"
                    class="rounded-lg px-2 py-2.5 inline-flex items-center w-full hover:bg-gray-200 focus:ring-2 focus:outline-none focus:ring-gray-300 dark:hover:bg-gray-600 dark:focus:ring-gray-500"
                    @click="toggleThemes(themeStore.getMode)">
